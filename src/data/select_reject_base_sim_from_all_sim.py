...
def main(
    ...
    with open(it_path, 'r') as f:
        for line in f:
            if line.strip():
                it_data.append(json.loads(line))
    ...
    for chunk_file in tqdm(chunk_files, desc="Reading Chunks"):
        with open(chunk_file, 'r') as f:
            ...
            print(f"Loaded {len(chunk_data)} items from {chunk_file}")
            for item in chunk_data:
                count = 0
                for it in it_data:
                    if(item["instruction"] == it["instruction"] and item["input"] == it["input"] and item["chosen"] == it["output"]):
                        if count >= 1:
                            print(f"Warning: Found multiple matches for item with instruction: {item['instruction']}, input: {item['input']}, chosen: {item['chosen']}. This may indicate duplicate entries in the SFT data.")
                            break
                        all_similarity_items.extend([item])
                        count += 1
                        

    try:
        if len(all_similarity_items) !=  len(it_data):
            print(f"Error: The number of similarity items ({len(all_similarity_items)}) does not match the number of SFT items ({len(it_data)}).")
            raise ValueError("Error: The number of similarity items does not match the number of SFT items.")
        else:
            print(f"Successfully loaded {len(all_similarity_items)} similarity items, matching the number of SFT items.")
    ...
    for key, data_list in results.items():
        print(f"key: {key}, output_dir: {output_dir}")
