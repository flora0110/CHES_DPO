...
def collect_results(metrics_root: str, metric_filter: Optional[List[str]] = None, percentiles: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    for metric_name in metric_dirs:
        for p in percentiles:
            eval_path = os.path.join(metrics_root, metric_name, p, "eval_top5.json")
...
def add_sft_deltas(df: pd.DataFrame, sft_eval_path: Optional[str]) -> pd.DataFrame:
    if not sft_eval_path:
        return df

    eval_records = load_eval_json(sft_eval_path)
    sft_row = parse_eval_top5(eval_records)

    out_df = df.copy()
    for field in CORE_FIELDS:
        if field in out_df.columns and field in sft_row:
            sft_val = numeric_or_none(sft_row[field])
            if sft_val is None:
                continue
            out_df[f"delta_{field}"] = out_df[field].apply(
                lambda x: None if numeric_or_none(x) is None else numeric_or_none(x) - sft_val
            )
    return out_df
...
def save_all_tables(df: pd.DataFrame, output_dir: str) -> None:
    ...
    # Delta head/tail table if SFT deltas exist
    delta_ht_cols = ["Method", "delta_NDCG_head@5", "delta_HR_head@5", "delta_NDCG_tail@5", "delta_HR_tail@5"]
    if all(col in df.columns for col in delta_ht_cols[1:]):
        save_markdown_table(
            df=df,
            out_path=os.path.join(output_dir, "head_tail_delta_from_sft.md"),
            columns=delta_ht_cols,
            sort_by=["metric", "percentile_value"],
        )
    ...
    for metric_name, subdf in df.groupby("metric"):
        ...
        if all(col in subdf.columns for col in delta_ht_cols[1:]):
            save_markdown_table(
                df=subdf,
                out_path=os.path.join(output_dir, f"{safe_metric}_head_tail_delta_from_sft.md"),
                columns=delta_ht_cols,
                sort_by=["percentile_value"],
            )


def plot_bar_comparisons(df: pd.DataFrame, output_dir: str, bar_fields: List[str], use_delta: bool = False) -> None:
    os.makedirs(output_dir, exist_ok=True)
    ...
    for metric_name, subdf in df.groupby("metric"):
        ...
        for field in bar_fields:
            actual_field = f"delta_{field}" if use_delta else field
            if actual_field in subdf.columns:
                resolved_fields.append(actual_field)

        if len(resolved_fields) == 0:
            continue

        n_fields = len(resolved_fields)
        fig, axes = plt.subplots(n_fields, 1, figsize=(10, 4 * n_fields))
        if n_fields == 1:
            axes = [axes]

        for ax, field in zip(axes, resolved_fields):
        ...
        fig.tight_layout()
        suffix = "delta_bar_comparison" if use_delta else "bar_comparison"
        out_path = os.path.join(output_dir, f"{metric_name}_{suffix}.png")
...
def plot_line_chart(
    ...
    actual_field = f"delta_{y_field}" if use_delta else y_field
    if actual_field not in plot_df.columns:
        raise ValueError(f"Field not found for plotting: {actual_field}")
    ...
    for metric_name in metric_names:
        ...
        y_vals = [numeric_or_none(v) for v in subdf[actual_field].tolist()]
        ax.plot(x_vals, y_vals, marker="o", label=metric_name)

    ax.set_xlabel("Preference Similarity (Percentile)")
    ax.set_ylabel(actual_field)
    ax.set_title(title if title else f"{actual_field} by percentile")
    ...
    fig.tight_layout()
    safe_field = re.sub(r"[^A-Za-z0-9_@]+", "_", actual_field)
    out_path = os.path.join(output_dir, f"line_{safe_field}.png")
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to SFT/base model eval_top5.json. If provided, the script will also compute and plot deltas from SFT.",
    )
    ...
    df = add_sft_deltas(df, args.sft_eval_path)
    ...
    plot_bar_comparisons(df, bar_dir, args.bar_fields, use_delta=False)
    plot_line_chart(
        df=df,
        output_dir=line_dir,
    ...
    if args.sft_eval_path:
        delta_bar_dir = os.path.join(plots_dir, "bar_delta_from_sft")
        delta_line_dir = os.path.join(plots_dir, "line_delta_from_sft")
        os.makedirs(delta_bar_dir, exist_ok=True)
        os.makedirs(delta_line_dir, exist_ok=True)

        plot_bar_comparisons(df, delta_bar_dir, args.bar_fields, use_delta=True)
        plot_line_chart(
            df=df,
            output_dir=delta_line_dir,
            y_field=args.line_field,
            metric_filter=args.metrics,
            title=f"Delta {args.line_field} from SFT by preference similarity percentile",
            use_delta=True,
        )
