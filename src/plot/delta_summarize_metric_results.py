    ...
    for metric_name in metric_dirs:
        for p in percentiles:
            ...
            if not os.path.exists(eval_path):
                print(f"[Warning] Missing: {eval_path}, try under 'final' subdir")
                
                eval_path = os.path.join(metrics_root, metric_name, p, "final" , "eval_top5.json")
                if not os.path.exists(eval_path):
...
def plot_line_chart(
    ...
    out_path = os.path.join(output_dir, f"delta_line_{safe_field}.png")
    fig.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
