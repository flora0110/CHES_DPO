...
# [MODIFIED] 移除了 name2genre 和 genre_dict 的讀取
...
# [MODIFIED] 移除了計算 Ground Truth History Genre Distribution (gh) 的函數
...
for topk in topk_list:
    ...
    for i in tqdm(range(len(test_data)),desc="Calculating Metrics......"):
        ...
        # [MODIFIED] 移除 Popularity bias & genre fairness 計算區塊
        ...
        # diversity (這個是基於 rank ID 的覆蓋率，與 genre 無關，保留)
        for i in range(topk):
...
# genre = args.category # 未使用，可忽略

# [MODIFIED] 移除 MGU/DGU 計算邏輯
...
# [MODIFIED] 移除 eval_dic 中的 DGU/MGU
...
print(count)
output_dir_dir = os.path.dirname(args.output_dir)
if output_dir_dir != "":
...
def update_csv(dataset_name, model_name, metrics_dict, csv_file):
    ...
    if not all(col in df.columns for col in required_columns):
        raise ValueError("CSV 文件必須包含 'Dataset' 和 'Model' 列")
    ...
    for metric, value in metrics_dict.items():
        if metric not in df.columns:
            print(f"注意：指標 '{metric}' 不在 CSV 文件列中，已添加該列並初始化為0。")
            df[metric] = 0  
...
if args.exp_csv != None:
    metric_dic = {}
    # [MODIFIED] 移除 metric_dic 中的 MGU/DGU
