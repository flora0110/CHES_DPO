...
model_it_dir="./experiments_new/models/${category}_sftlr_${sft_lr}_lr_${lr}_beta${beta}/it${i}"
predicts_it_dir="./experiments_new/predicts/${category}_sftlr_${sft_lr}_lr_${lr}_beta${beta}/it${i}"
metrics_it_dir="./experiments_new/metrics/${category}_sftlr_${sft_lr}_lr_${lr}_beta${beta}/it${i}"
metrics_base_dir="./experiments_new/metrics/${category}_sftlr_${sft_lr}_lr_${lr}_beta${beta}" # 這個是給後續agg_eval.py用的，因為它需要讀取所有iteration的eval_top5.json來做彙總
...
if [ ! -f "${DPO_model_path}/adapter_config.json" ]; then
    CUDA_VISIBLE_DEVICES=$gpu1 python ./src/models/dpo.py \
        ...
        --eval_strategy "epoch" \
        --save_strategy "epoch" \
        --sh_file_path "./shell/single_SFT_DPO_runner.sh" \
